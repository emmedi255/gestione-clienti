...
export function FornitoriSection({ userId, addedFornitori, setAddedFornitori }) {
...
const addSelectedFornitore = () => {
  ...
  if (!forn) return;
  if (addedFornitori.some((f) => String(f.fornitore_id) === String(forn.fornitore_id))) return;
 ...
 const deleteSelectedFornitore = async () => {
  ...
  try {
    const res = await fetch(
      `/api/fornitori?id=${selectedFornitoreId}`,
      { method: "DELETE" }
    );
      ...
      <table className="min-w-full border-collapse border border-gray-300 rounded-lg overflow-hidden shadow-sm">
        <thead className="bg-blue-100">
          <tr>
            ...
            <th className="px-4 py-2 text-left font-medium text-gray-700">Indirizzo</th>
            <th className="px-4 py-2 text-left font-medium text-gray-700">CF/P.IVA</th>
            <th className="px-4 py-2 text-left font-medium text-gray-700">Attività</th>
            <th className="px-4 py-2 text-left font-medium text-gray-700">Azioni</th>
          </tr>
        </thead>
        <tbody>
          {addedFornitori.map((f, i) => (
            <tr key={f.fornitore_id} className={i % 2 === 0 ? "bg-white" : "bg-gray-50 hover:bg-gray-100"}>
  ...
  {showModal && (
    <div className="fixed inset-0 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg w-full max-w-md p-6 relative shadow-lg">
        ...
        {["nome","indirizzo","cap","provincia","cf","attivita"].map((field) => (
          <input
            key={field}
            type="text"
            placeholder={field === "nome" ? "Ragione sociale / Nome Cognome" :
                      field === "cf" ? "CF / P.IVA" :
                      field === "attivita" ? "Tipo attività" : field}
            value={newFornitore[field]}
            onChange={(e) => setNewFornitore((prev) => ({ ...prev, [field]: e.target.value }))}
