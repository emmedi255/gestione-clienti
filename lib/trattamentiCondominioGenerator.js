...
const buildSezione03Frase = (sezione03) => {
  ...
  if (sezione03.elettronica?.enabled) {
    ...
    if (sezione03.elettronica?.serverLocale?.checked) {
      const auth = sezione03.elettronica.serverLocale.autenticazione ? "con autenticazione" 
              : sezione03.elettronica.serverLocale.noAutenticazione ? "senza autenticazione" : "";
      eleCloud.push(`server locale ${auth}`);
    }
    if (sezione03.elettronica?.cloud?.checked) {
      const auth = sezione03.elettronica.cloud.autenticazione ? "con autenticazione" 
              : sezione03.elettronica.cloud.noAutenticazione ? "senza autenticazione" : "";
  ...
  if (sezione03.elettronica?.serverLocale?.password) sicurezzaParts.push("password server locale");
  
  if (sezione03.elettronica?.cloud?.password) sicurezzaParts.push("password cloud");
  ...
  if (sezione03.sicurezza?.cambioPassword) genSicurezza.push("cambio credenziali");
  ...
  if (sezione03.elettronica.serverLocale?.altro) {
    altroElettronica.push(`"${sezione03.elettronica.serverLocale.altro.trim()}" (server locale)`);
  }
  if (sezione03.elettronica.cloud?.altro) {
    altroElettronica.push(`"${sezione03.elettronica.cloud.altro.trim()}" (cloud)`);
  }
  if (altroElettronica.length > 0) {
    parts.push(`altre misure: ${altroElettronica.join("; ")}`);
  }

  return parts.length > 0 ? parts.join("; ") + "." : "Nessuna informazione specificata.";
};
