...
qualità di “Titolare del Trattamento” (art. 4), il </p>

  <p><strong>${intestazione.condominio || 'Condominio'}</strong></p>
  <p><strong> ${intestazione.cap || ''} ${intestazione.citta || ''}  ${intestazione.provincia || ''} C.F. ${intestazione.cfCondominio || ''}</strong></p>
...
">
  <p>${amministratore || ""}</p>
  <p>${studio || ""}</p>
  <p>${sezione07.indirizzoStudio || ""} </p>
  <p>${user.cap_studio} ${user.citta_studio} (${user.pr_studio})</p>
</div>
  ...
  <p><strong>in qualità di Amministratore di condominio </strong></p>
...
condominio e di tutti gli adempimenti normativi ad esso connessi. </p>
  ...
  <p>Il Responsabile così individuato e nominato, nello svolgimento di tale incarico, effettuato
con strumenti cartacei, elettronici o comunque automatizzati o con strumenti diversi, in
relazione ai trattamenti dei dati rientranti nell’ambito operativo e funzionale di propria
competenza, si obbliga a:</p>
